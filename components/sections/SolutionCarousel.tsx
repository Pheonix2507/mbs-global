...
const SolutionCarousel = ({
  ...
  // Extract header info
  const headerTitle =
    propTitle || data?.title || data?.title_subtile?.title || "";
  const headerDescription =
    propDescription ||
    data?.sub_title ||
    data?.subtitle ||
    data?.description ||
    data?.title_subtile?.sub_title ||
    "";
    ...
    <section className="bg-zinc-50 dark:bg-zinc-900/50 py-24">
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="font-zalando text-3xl md:text-5xl font-semibold tracking-tight text-[#D699FF] dark:text-[#D699FF] mb-6">
            {headerTitle}
          </h2>
          <p className="font-sans text-lg text-black dark:text-white">
            {headerDescription}
          </p>
        </div>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="w-full md:px-20">
            <div className="bg-black rounded-[4px] px-4 py-8 sm:px-8 md:p-16 shadow-2xl border border-white/10 text-left md:text-center transform transition-all duration-500 min-h-[300px] flex flex-col justify-center">
              <h3
                className="font-zalando text-3xl md:text-4xl font-semibold tracking-tight text-white mb-6 animate-in fade-in zoom-in duration-500 wrap-break-words leading-[110%]"
                key={`title-${currentIndex}`}
              >
                {displayData[currentIndex].title}
              </h3>
              <p
                className="font-sans text-base md:text-xl text-zinc-400 leading-relaxed max-w-2xl md:mx-auto animate-in fade-in zoom-in duration-500 delay-100"
